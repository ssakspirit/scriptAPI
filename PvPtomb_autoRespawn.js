 ...
 * 4. 자동부활 기능 코드가 추가되어 있습니다. 10초 후 자동으로 부활합니다.
 * 
 * 관리자 명령어:
...
world.afterEvents.entityDie.subscribe((event) => {
    ...
    if (entity.typeId === "minecraft:player") {
        ...
        // 10초 후 자동 부활 타이머 설정 코드 시작
        system.runTimeout(() => {
            try {
                // 플레이어가 여전히 온라인이고 묘비가 존재하는지 확인
                const onlinePlayer = world.getAllPlayers().find(p => p.name === entity.name);
                if (!onlinePlayer || !tombMap.has(entity.name)) return;
                ...
                // 플레이어 부활 처리
                entity.teleport(tombMap.get(entity.name).location);
                entity.runCommandAsync("gamemode survival @s");
                entity.nameTag = entity.name;
                entity.sendMessage("§a10초가 지나 자동으로 부활되었습니다!");

                // 모든 플레이어에게 알림
                world.getAllPlayers().forEach(p => {
                    p.sendMessage(`§e${entity.name}님이 자동으로 부활했습니다!`);
                });

                // 인터벌 및 매핑 제거
                if (tombMap.has(entity.name)) {
                    system.clearRun(tombMap.get(entity.name).interval);
                    tombMap.delete(entity.name);
                }
                ...
                console.warn("자동 부활 처리 중 오류 발생:", error);
            }
        }, 200); // 10초 = 200틱
        //자동 부활 코드 종료
