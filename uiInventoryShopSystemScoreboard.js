...
const SHOP_CONFIG = {
    ...
    MONEY_OBJECTIVE: "Money"                  // 화폐로 사용할 스코어보드 이름
};
...
// 아이템 구매 처리 (잔액 확인은 API, 나머지는 runCommand)
function buyItem(player, item, amount) {
    const totalPrice = item.price * amount;
    // 머니 잔액 확인 (API)
    let money = 0;
    try {
        const obj = world.scoreboard.getObjective(SHOP_CONFIG.MONEY_OBJECTIVE);
        if (obj) {
            money = obj.getScore(player.scoreboardIdentity) ?? 0;
        }
    } catch {}
    if (money < totalPrice) {
        player.sendMessage(`§c머니가 부족합니다. §e(보유: ${money}, 필요: ${totalPrice})`);
        return;
    }
    // 머니 차감
    ...
    saveShopData();
    player.sendMessage(`§a${item.name} ${amount}개를 구매했습니다. §e(${totalPrice} 머니 사용)`);
}
...
// 아이템 판매 처리 (잔액 확인은 API, 나머지는 runCommand)
function sellItem(player, item, amount) {
    // 아이템 개수 확인 (API로는 생략, 실패 시 메시지)
    // 아이템 제거
    const clearResult = player.runCommand(`clear @s ${item.id} 0 ${amount}`);
    if (clearResult.successCount === 0) {
        player.sendMessage(`§c${item.name}이(가) 부족합니다. (필요: ${amount}개)`);
        return;
    }
    // 머니 지급
    const sellPrice = Math.floor(item.price * SHOP_CONFIG.SELL_PRICE_RATIO * amount);
    player.runCommand(`scoreboard players add @s ${SHOP_CONFIG.MONEY_OBJECTIVE} ${sellPrice}`);
    item.stock += amount;
    saveShopData();
    player.sendMessage(`§a${item.name} ${amount}개를 판매했습니다. §e(${sellPrice} 머니)`);
}
...
world.beforeEvents.chatSend.subscribe(event => {
    ...
    if (message === "!상점") {
        ...
        showAdminUI(player);
    } else if (message === "!상점초기화" && player.hasTag("admin")) {
        world.setDynamicProperty("shopData", undefined);
        initializeShop();
        saveShopData();
        player.sendMessage("§a상점이 기본값으로 초기화되었습니다.");
        event.cancel = true;
    }
});
