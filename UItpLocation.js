...
function showForm(player) {
    ...
    formData.show(player).then((response) => {
        ...
        if (response.selection === 0) {
            ...
            player.runCommand("title @a actionbar 마을 위치로 이동 완료");
        }
        ...
        if (response.selection === 1) {
            ...
            playerPositionZ = 10;
            player.runCommand(`tp @s ${playerPositionX} ${playerPositionY} ${playerPositionZ}`)
            player.runCommand("title @a actionbar 상점 위치로 이동 완료");
        }
        ...
        if (response.selection === 2) {
            ...
            playerPositionZ = 20;
            player.runCommand(`tp @s ${playerPositionX} ${playerPositionY} ${playerPositionZ}`)
            player.runCommand("title @a actionbar 동굴 위치로 이동 완료");
        }
    });
}
