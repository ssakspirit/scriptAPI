    ...
    try {
        ...
        try {
            ...
            try {
                ...
                if (currentItem) {
                    const prefix = ENHANCEMENT_PREFIXES[nextLevel];
                    currentItem.nameTag = `${currentItem.typeId.replace('minecraft:', '').replace(/_/g, ' ')} ${prefix}`;
                    inventory.container.setItem(selectedSlot, currentItem);
                }
            } catch (error) {
                console.warn("아이템 이름 설정 중 오류 발생:", error);
            }
...
function getSelectedItem(player) {
    try {
        ...
        console.warn("아이템 가져오기 중 오류:", error);
        return undefined;
    }
}
