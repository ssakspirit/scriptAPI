...
// 몬스터 수를 저장할 스코어보드 생성 (API 사용)
try {
    if (!world.scoreboard.getObjective("warden_count")) {
        world.scoreboard.addObjective("warden_count", "워든 수");
    }
    if (!world.scoreboard.getObjective("zombie_count")) {
        world.scoreboard.addObjective("zombie_count", "좀비 수");
    }
} catch (error) {
    console.warn("스코어보드 생성 중 오류:", error);
}
